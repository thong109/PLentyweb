...
//        dump($searchData);die;
        ...
        // SaleTypeId
        if (isset($searchData['sale_type_id'])) {
            $qb->andWhere('pc.SaleType = :SaleType')
                ->setParameter('SaleType', $searchData['sale_type_id']);
        }

        // highlight
        if (isset($searchData['highlight'])) {
            $qb->andWhere('p.highlight = :highlight')
                ->setParameter('highlight', (int) $searchData['highlight']);
        }
    ...
    public function getQueryBuilderBySearchDataForAdmin($searchData)
    {
        $qb = $this->createQueryBuilder('p')
            ...
            ->andWhere('pc.visible = :visible')
            ->andWhere('pc.SaleType != :SaleTypeSeminar')
            ->setParameter('SaleTypeSeminar', SaleType::SALE_TYPE_SEMINAR)
            ->setParameter('visible', true);
    ...
    /**
     * @functionName getTopPage
     * @author vungpv93@gmail.com
     * @return int|mixed|string
     */
    public function getTopPage($sale_type_id = null, $highlight = null, $mode = null)
    {
        $qb = $this->createQueryBuilder('p');
//        $qb->select(['p'])
//        $qb->addSelect(['pc', 'cc1', 'cc2', 'pi', 'pt', 'tr', 'ps'])
...
//            ->innerJoin('pc.ProductStock', 'ps')
        ...
        // saletype status
        if (isset($sale_type_id) && !empty($sale_type_id)) {
            $qb
                ->andWhere('pc.SaleType = :SaleType')
                ->setParameter('SaleType', $sale_type_id);
        }

        if (!empty($mode) && $mode == 'commingsoon') {
//            $qb->addSelect('pm');
            $qb->leftJoin('p.ProductSeminar', 'pm');
            $qb->andWhere(
                $qb->expr()->isNotNull('pm.startTime'),
                $qb->expr()->gte('pm.startTime', ':startTime')
            );
            $qb->setParameter('startTime', new \DateTime('now'));

//            $qb->orderBy('pm.startTime', 'ASC');
            $qb->orderBy('p.create_date', 'DESC');
            $qb->addOrderBy('p.id', 'DESC');
        } elseif ($sale_type_id == 20) {
//            $qb->groupBy('p.id');
            $qb->orderBy('p.create_date', 'DESC');
            $qb->addOrderBy('p.id', 'DESC');
        } else {
