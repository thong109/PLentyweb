 ...
 */
class SeminarRepository extends \Eccube\Repository\AbstractRepository
{
    /**
     * @var Queries
     */
    ...
    public function getQueryBuilderBySearchData($searchData)
    {
        $qb = $this->createQueryBuilder('p')
            ...
            ->setParameter('SaleType', ProductEntity::SEMINAR_PRODUCT);

        if (isset($searchData['mode']) && !empty($searchData['mode']) && $searchData['mode'] == 'commingsoon') {
            $qb->leftJoin('p.ProductSeminar', 'ps');
            $qb->andWhere(
                $qb->expr()->isNotNull('ps.startTime'),
                $qb->expr()->gte('ps.startTime', ':startTime')
            );
            $qb->setParameter('startTime', new \DateTime('now'));
            if (!empty($searchData['start_time']) && $searchData['start_time']) {
                $qb->andWhere('ps.startTime LIKE :Start_time')
                    ->setParameter('Start_time', '%'.$searchData['start_time']."%");
            }

            $qb->orderBy('ps.startTime', 'ASC');
        } else {
            if (!empty($searchData['start_time']) && $searchData['start_time']) {
                $qb->leftJoin('p.ProductSeminar', 'ps')
                    ->andWhere('ps.startTime LIKE :Start_time')
                    ->setParameter('Start_time', '%'.$searchData['start_time']."%");
                $qb->orderBy('ps.startTime', 'ASC');
            }
        }
//
        ...
        if (isset($searchData['highlight']) && !empty($searchData['highlight'])) {
            $qb->andWhere('p.highlight =:highlight');
            $qb->setParameter('highlight', $searchData['highlight']);
        }
    ...
    public function getQueryBuilderBySearchDataForAdmin($searchData)
    {
        ...
        // store
        if (!empty($searchData['store_id']) && $searchData['store_id']) {
            $qb
                ->andWhere($qb->expr()->in('p.Store', ':Store'))
                ->setParameter('Store', $searchData['store_id']);
        }
...
//
    ...
    /**
     * Find the new seminars.
     *
     * @return ArrayCollection|array
     */
    public function findNewSeminar()
    {
        $qb = $this->createQueryBuilder('p');
        $qb->addSelect(['pc', 'cc1', 'cc2', 'pi', 'ps'])
            ->innerJoin('p.ProductClasses', 'pc')
            ->innerJoin('p.ProductSeminar', 'ps')
            ->leftJoin('pc.ClassCategory1', 'cc1')
            ->leftJoin('pc.ClassCategory2', 'cc2')
            ->leftJoin('p.ProductImage', 'pi')
            ->andWhere('p.Status = 1')
    ...
    /**
     * Find seminars by schedule recently.
     *
     * @return ArrayCollection|array
     */
    public function findSeminarByScheduleRecently()
    {
        $qb = $this->createQueryBuilder('p');
        $qb->addSelect(['pc', 'cc1', 'cc2', 'pi', 'ps'])
            ->innerJoin('p.ProductSeminar', 'ps')
