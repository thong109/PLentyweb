...
class SaleTypeCartAllocator implements CartItemAllocator
{
    ...
    public function allocate(CartItem $Item)
    {
        ...
        if (
            ...
            && $ProductClass->getProduct()->getStore()
            && $ProductClass->getProduct()->getStore()->getId()
        ) {
            return (string) $ProductClass->getProduct()->getStore()->getId();
        }
        throw new \InvalidArgumentException('ProductClass/Store not found');
    }
