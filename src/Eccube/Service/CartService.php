...
class CartService
{
     ...
     */
    public function addProduct($ProductClass, $quantity = 1, $scheduleId, $saleType = 0, $minDay = 0, $startDate = null )
    {
        ...
        if($saleType == ProductEntity::RENTAL_PRODUCT) {
            $price01 = $ProductClass->getPrice01();
            $price02 = $ProductClass->getPrice02();
            $price01_inc_tax = $ProductClass->getPrice01IncTax();
            $price02_inc_tax = $ProductClass->getPrice02IncTax();
            $ProductClass->setPrice01($price01* $minDay);
            $ProductClass->setPrice02($price02* $minDay);
            $ProductClass->setPrice01IncTax($price01_inc_tax* $minDay);
            $ProductClass->setPrice02IncTax($price02_inc_tax* $minDay);

        }
        $newItem->setQuantity($quantity);
        $newItem->setProductClass($ProductClass);
        $newItem->setPrice($ProductClass->getPrice02IncTax());
        $newItem->setRentalMinDay($minDay);
        $newItem->setRentalStartDate($startDate);
        $newItem->setScheduleId($scheduleId);
